...
def open_page(request: OpenPage):
    try:
        if request.dataset_id:
            ...
            machine, simulated = find_machine(request.machine_id), get_data_source() == 'mock'
        if machine is None or machine.machine_id != request.machine_id:
            raise LookupError()
    ...
    if machine.serial_number.strip().upper() != request.vin:
