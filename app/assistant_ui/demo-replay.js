/**
 * Replay a real saved AI investigation, beside the scripted demo case.
 *
 * The scripted case is a simulation and says so; it cannot show what the model
 * does. This module presents genuine saved model output instead, read straight
 * from the local records, so a reviewer sees the real thing without an API call
 * and without mistaking the recording for a live run.
 *
 * It renders only what the backend replay payload contains: no wording about the
 * AI is written here, so the page cannot claim more than the record shows.
 */
(() => {
  ...
  let listing = null, current = null, loading = null, mode = 'scripted';

  function showMode(next) {
    mode = next === 'real' ? 'real' : 'scripted';
    const real = mode === 'real';
    byId('demo-mode-scripted').setAttribute('aria-pressed', String(!real));
    byId('demo-mode-real').setAttribute('aria-pressed', String(real));
    byId('replay-view').hidden = !real;
    // The scripted case owns its own content node, so hand it back explicitly
    // rather than trying to guess whether it has finished loading.
    if (typeof updateDemoDisclosure === 'function') updateDemoDisclosure();
    if (real) {
      byId('demo-content').hidden = true;
      byId('demo-status').hidden = true;
      loadListing();
    } else if (typeof window.enterDemoCase === 'function') {
  ...
  function render(data) {
    ...
    if (exportLink) exportLink.href = '/assistant/demo-case/replay/' + encodeURIComponent(data.record_id) + '/report.md';
  }
  ...
  async function openRecord(recordId) {
    ...
    try {
      const data = await api('/assistant/demo-case/replay/' + encodeURIComponent(recordId));
      render(data);
    } catch (error) {
  ...
  function loadListing() {
    ...
    loading = api('/assistant/demo-case/replays').then(data => {
      listing = data;
  ...
  byId('demo-mode-scripted').onclick = () => showMode('scripted');
  byId('demo-mode-real').onclick = () => showMode('real');
  byId('replay-picker').onchange = event => openRecord(event.target.value);
  window.showReplayMode = showMode;
})();
