...
const AIRequestStatus = (()=>{
  ...
  function view(data){
    ...
    if(data.configured===false)return {title:'Gemini 密钥未配置',tone:'warning',time:null,
    ...
    else if(last.outcome==='failed'){title=prefix+(failureNames[last.failure?.kind]||'未完成');tone='warning';}
