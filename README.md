...
网页与侧栏采用徐工蓝白品牌栏、官方字标和紧凑工作台布局。设备信息、运行趋势和 AI 排查为主入口；版本与来源详情、分析设置、草稿和近期请求记录可按需展开。模拟标识、真实错误、缺失数据和诊断待核验状态继续保留。浏览器图标也已替换，已安装插件需要重新加载到 0.6.2。

## 当前设备自动关联

当前网页构建：`20260915.10-xcmg-workspace`；Chrome 插件源码：`0.6.2`。工作预览在 [8890 设备排查](http://127.0.0.1:8890/assistant-ui/)。升级后在 Chrome 扩展管理页重新加载插件，再重新打开侧栏；仅点击侧栏“重新连接”不会更新插件代码。

插件跟随当前 Trackunit 详情页的设备 UUID，并提取标签页标题中显示的设备编号作为查询提示。网页优先载入该机已有实测数据，没有本地版本时，优先用编号查询 AEMP 单机快照，并要求返回的 `metadata.assetId` 与页面 UUID 完全一致。没有提示时尝试 Assets 接口及最多 3 页的 AEMP 车队快照。只独立保存目标设备，不覆盖整队缓存；身份冲突、缺少有效采样时间或访问失败时显示原因。本次自动读取不调用 AI 或故障事件接口。
...
| Chrome 侧栏 | 0.6.2 更新品牌与展示布局；0.6.1 修复演示与跟随状态同步，新增未导入设备的 API 读取；保留 XGSS 可见条目读取；实际重载及侧栏操作待验收 |
| 工况与冷却预警实验 | 附带模拟回放所需模型和测试片段，未完成实机效果验证 |

此次插件源码升级到 `0.6.2`；已安装旧版时，需要在 Chrome 扩展页重新加载项目的 `extension` 文件夹。连接 8890，或由新版插件选择构建兼容的端口。仅重新连接网页不能更新扩展脚本。保存未提交草稿后再操作。自动跟随会为没有本地数据的设备发起有界只读请求，不自动调用 AI；XGSS 条目由用户点击读取，覆盖范围仅限当前可见页面。

## 开发与验证

当前完整测试：Python **665 项通过**、Node **144 项通过**。Chrome 网页已显示 XC918PRO 的 2 条、XC948U 的 3 条真实快照记录；这些是带时间戳的快照通道，不代表连续历史或故障记录。见[设备关联验收证据](docs/evaluation/platform-follow-2026-09-15.json)。此前测试故障、真实 AI 请求、模拟反馈续查、历史重开和报告下载见 [XE55U 验收证据](docs/evaluation/xe55u-first-case-2026-09-15.json)。XGSS 真实料号及新版扩展实装仍未验收。
