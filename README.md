...
当前构建：`20260915.7-auto-device-data`。
...
| Chrome 侧栏 | 自动识别并跟随 Trackunit 设备，载入同一 UUID 的实测数据；保留已有版本选择，否则按最新有效采样选择，仍可手动切换；本次修复的浏览器验收见当前验收状态 |
| 工况与冷却预警实验 | 附带模拟回放所需模型和测试片段，未完成实机效果验证 |

本次修改网页中的数据版本选择流程，插件仍为 `0.5.0`。已安装该版本时，无需重新安装；先保存未提交草稿，再在侧栏点击“重新连接”载入新版网页。多个版本采样相同时稳定选择一个；日期未知也提供默认版本并说明，不再停在“请选择数据版本”。自动选择只读取同一设备的已有实测资料，不混入模拟数据或其他设备，也不自动调用 AI 或同步 Trackunit。
