...
当前网页构建：`20260915.9-platform-follow`；Chrome 插件源码：`0.6.1`。工作预览在 [8890 设备排查](http://127.0.0.1:8890/assistant-ui/)。升级后在 Chrome 扩展管理页重新加载插件，再重新打开侧栏；仅点击侧栏“重新连接”不会更新插件代码。
...
| Chrome 侧栏 | 0.6.1 修复演示与跟随状态同步，新增未导入设备的 API 读取；保留 XGSS 可见条目读取；实际重载及侧栏操作待验收 |
| 工况与冷却预警实验 | 附带模拟回放所需模型和测试片段，未完成实机效果验证 |

此次插件源码升级到 `0.6.1`；已安装旧版时，需要在 Chrome 扩展页重新加载项目的 `extension` 文件夹。连接 8890，或由新版插件选择构建兼容的端口。仅重新连接网页不能更新扩展脚本。保存未提交草稿后再操作。自动跟随会为没有本地数据的设备发起有界只读请求，不自动调用 AI；XGSS 条目由用户点击读取，覆盖范围仅限当前可见页面。
